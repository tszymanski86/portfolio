...
function showSection() {
  ...
  window.onscroll = function () {
    Sections.forEach(function (section, index) {
      ...
      if (
        window.pageYOffset + window.innerHeight >= section.offsetTop + section.clientHeight
      )
...
const Slider = {
  ...
  showSlides: function (n) {
    ...
    if (n >= Slider.logos.length) {
      Slider.slideIndex = 0;
    }
    if (n < 0) {
      Slider.slideIndex = Slider.logos.length - 1;
    }
    for (i = 0; i < Slider.logos.length; i++) {
      Slider.logos[i].style.display = "none";
    }
    for (i = 0; i < Slider.miniImg.length; i++) {
      Slider.miniImg[i].className = Slider.miniImg[i].className.replace(
        " active",
        ""
      );
    }
  ...
  function startSnake() {
    ...
    function controls() {
      window.addEventListener(
        ...
        function (event) {
          if (dx)
            if (event.keyCode == 38 || event.keyCode == 40) key = event.keyCode;
          if (dy)
            if (event.keyCode == 37 || event.keyCode == 39) key = event.keyCode;
        },
        false
      );
    ...
    function print() {
      ...
      function findBodyFragment(nrSegmentu) {
        ...
        } else if (nrSegmentu == body.length - 1) {
          //ogon
          if (body[nrSegmentu][1] > body[nrSegmentu - 1][1]) {
            // w górę
            xFragment = 192;
            yFragment = 128;
          } else if (body[nrSegmentu][1] < body[nrSegmentu - 1][1]) {
            // w dół
            xFragment = 256;
            yFragment = 192;
          }
          if (body[nrSegmentu][0] > body[nrSegmentu - 1][0]) {
            // w lewo
            xFragment = 192;
            yFragment = 192;
          } else if (body[nrSegmentu][0] < body[nrSegmentu - 1][0]) {
            // w prawo
            xFragment = 256;
            yFragment = 128;
          }
        } else if (nrSegmentu > 0 && nrSegmentu < body.length - 1) {
          //część środkowa
          if (
            body[nrSegmentu][0] == body[nrSegmentu + 1][0] &&
            body[nrSegmentu][0] == body[nrSegmentu - 1][0]
          ) {
            // pion
            xFragment = 128;
            ...
            body[nrSegmentu][1] == body[nrSegmentu + 1][1] &&
            body[nrSegmentu][1] == body[nrSegmentu - 1][1]
          ) {
            // poziom
            xFragment = 64;
            ...
            (body[nrSegmentu - 1][1] > body[nrSegmentu][1] &&
              body[nrSegmentu + 1][0] > body[nrSegmentu][0]) || // w górę i prawo / w lewo i dół
            (body[nrSegmentu + 1][1] > body[nrSegmentu][1] &&
              body[nrSegmentu - 1][0] > body[nrSegmentu][0])
          ) {
            xFragment = 0;
            yFragment = 0;
          } else if (
            (body[nrSegmentu - 1][0] > body[nrSegmentu][0] &&
              body[nrSegmentu + 1][1] < body[nrSegmentu][1]) || // w lewo i do góry
            (body[nrSegmentu + 1][0] > body[nrSegmentu][0] &&
              body[nrSegmentu - 1][1] < body[nrSegmentu][1])
          ) {
            xFragment = 0;
            ...
            (body[nrSegmentu - 1][1] < body[nrSegmentu][1] &&
              body[nrSegmentu + 1][0] < body[nrSegmentu][0]) || // w dół i lewo
            (body[nrSegmentu + 1][1] < body[nrSegmentu][1] &&
              body[nrSegmentu - 1][0] < body[nrSegmentu][0])
          ) {
            xFragment = 128;
            ...
            (body[nrSegmentu - 1][0] < body[nrSegmentu][0] &&
              body[nrSegmentu + 1][1] > body[nrSegmentu][1]) || // w prawo i dół
            (body[nrSegmentu + 1][0] < body[nrSegmentu][0] &&
              body[nrSegmentu - 1][1] > body[nrSegmentu][1])
          ) {
            xFragment = 128;
    ...
    function eatenFruit() {
      function findCollision() {
        for (let i = 0; i < body.length; i++) {
          if (body[i][0] == fruitX && body[i][1] == fruitY) {
            return true;
          }
        }
        return false;
    ...
    let move = setInterval(function () {
      ...
      if (
        body[0][0] < 0 ||
        body[0][0] + SizeCan > CanWidth ||
        body[0][1] < 0 ||
        body[0][1] + SizeCan > CanHeight
      ) {
        gameOver = true;
      }
